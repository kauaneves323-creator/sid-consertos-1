...
(function () {
  ...
  const FIN_STORAGE_KEY = 'sidConsertos_financas_v1';

  const FIN_CATEGORIAS = ['Peça','Mão de obra','Deslocamento/Combustível','Ferramenta/Equipamento','Conta fixa','Outro'];
  ...
  // --- Financeiro (calendário mensal de peças, mão de obra, despesas etc.) ---
  let financas = [];            // lançamentos manuais (entradas/saídas extras)
  let finCursorDate = new Date();  // mês/ano exibido no calendário
  let finSelectedDay = null;    // dia (ISO) selecionado no painel de detalhe
  let finEditingLancamentoId = null;
  ...
  function showToast(message, isError, action) {
    ...
    let html = `<i class="fa-solid ${isError ? 'fa-circle-xmark' : 'fa-circle-check'}"></i><span>${escapeHtml(message)}</span>`;
    if (action && action.label && typeof action.onClick === 'function') {
      html += `<button type="button" class="toast-action" id="toastActionBtn">${escapeHtml(action.label)}</button>`;
    }
    ...
    showToast._t = setTimeout(() => toast.classList.remove('show'), action ? 5000 : 2600);
  }
  ...
  function loadFinancas() {
    try {
      ...
      financas = raw ? JSON.parse(raw) : [];
      if (!Array.isArray(financas)) financas = [];
    } catch (e) {
  ...
  function saveFinancas() {
    try {
      localStorage.setItem(FIN_STORAGE_KEY, JSON.stringify(financas));
      return true;
    } catch (e) {
      console.error('Erro ao salvar lançamentos financeiros', e);
      showToast('Não foi possível salvar o lançamento. Armazenamento cheio ou indisponível.', true);
      return false;
    }
  }
  ...
  /* ---------------------------------------------------------------
     NOVO ATENDIMENTO (MESMA PESSOA) — dentro da MESMA chamada
     Não cria uma chamada nova (não gera número novo, não é um novo
     registro separado). Apenas adiciona um novo "atendimento" na
     lista de atendimentos desta chamada, aparecendo embaixo das
     informações já existentes na tela de detalhe. O status atual
     da chamada é atualizado para refletir esse atendimento mais
     recente.
  --------------------------------------------------------------- */
  function scrollToInteracoes() {
    const el = document.getElementById('detailInteracoesCard');
    if (el) el.scrollIntoView({ behavior: 'smooth', block: 'start' });
    const first = document.getElementById('mp_data');
    if (first) first.focus();
  }

  function renderInteracoesSection(call) {
    const interacoes = call.interacoes || [];

    const listHtml = interacoes.length === 0
      ? '<span class="detail-empty">Nenhum atendimento adicional registrado ainda.</span>'
      : `<div class="history-list" style="margin-bottom:14px;">
          ${interacoes.map(it => `
            <div class="history-item mp-interacao-item" data-iid="${it.id}">
              <span><b>${formatDateBR(it.data)}</b>${it.horario ? ' ' + it.horario : ''} — ${escapeHtml(it.tipoAtendimento || '')} — ${escapeHtml((it.reclamacao || 'Sem detalhes registrados').slice(0,60))}${(it.reclamacao||'').length>60?'…':''}</span>
              <span style="display:flex; align-items:center; gap:10px;">
                <span class="status-badge status-${slugStatus(it.status)}">${escapeHtml(it.status)}</span>
                <button type="button" class="icon-btn mp-interacao-del" data-iid="${it.id}" title="Apagar este atendimento"><i class="fa-solid fa-trash"></i></button>
              </span>
            </div>
          `).join('')}
        </div>`;
    ...
    return `
      <div class="detail-card" id="detailInteracoesCard">
        <h4><i class="fa-solid fa-clock-rotate-left"></i> Atendimentos desta chamada (mesma pessoa)</h4>
        ${listHtml}
        <div class="mp-inline-form">
          <div class="field-grid cols-3">
            <div class="field">
              <label>Data</label>
              <input type="date" id="mp_data" value="${todayISO()}">
            </div>
            <div class="field">
              <label>Horário</label>
              <input type="time" id="mp_horario">
            </div>
            <div class="field">
              <label>Tipo de atendimento</label>
              <select id="mp_tipoAtendimento">
          ...
          <div class="field field-highlight">
            <label><i class="fa-solid fa-quote-left"></i> Reclamação / motivo deste novo atendimento</label>
            <textarea id="mp_reclamacao" rows="2" placeholder="Ex: Cliente ligou de novo, agora relatando outro problema na mesma máquina."></textarea>
          </div>
          <div class="field">
            <label>Observação <span class="optional">(opcional)</span></label>
            <textarea id="mp_obsGerais" rows="2"></textarea>
          </div>
          <button type="button" class="btn btn-primary btn-sm" id="btnAddInteracao"><i class="fa-solid fa-plus"></i> Adicionar atendimento</button>
        </div>
      </div>
    `;
  }

  function wireInteracoesSection(callId) {
    const statusSel = document.getElementById('mp_status');
    const call = calls.find(c => c.id === callId);
    if (statusSel && call) statusSel.value = call.status;

    document.querySelectorAll('.mp-interacao-del').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        deleteInteracao(callId, btn.dataset.iid);
      });
    });

    const addBtn = document.getElementById('btnAddInteracao');
    if (addBtn) addBtn.addEventListener('click', () => addInteracao(callId));
  }

  function addInteracao(callId) {
    const call = calls.find(c => c.id === callId);
    if (!call) { showToast('Chamada não encontrada.', true); return; }

    const data = document.getElementById('mp_data').value;
    if (!data) { showToast('Informe a data do atendimento.', true); return; }

    const entry = {
      id: uid(),
      data,
      horario: document.getElementById('mp_horario').value,
      tipoAtendimento: document.getElementById('mp_tipoAtendimento').value,
      status: document.getElementById('mp_status').value,
      reclamacao: document.getElementById('mp_reclamacao').value.trim(),
      obsGerais: document.getElementById('mp_obsGerais').value.trim(),
      criadoEm: new Date().toISOString()
    };

    if (!call.interacoes) call.interacoes = [];
    call.interacoes.push(entry);

    // reflete o status desse atendimento mais recente na chamada como um todo
    const statusAnterior = call.status;
    call.status = entry.status;
    call.atualizadoEm = new Date().toISOString();

    if (!saveData()) return;

    refreshDashboard();
    openDetail(callId); // re-renderiza o detalhe já com o novo atendimento na lista

    showToast('Atendimento adicionado a esta chamada.', false, {
      label: 'Desfazer',
      onClick: () => {
        const c = calls.find(x => x.id === callId);
        if (c) {
          c.interacoes = (c.interacoes || []).filter(it => it.id !== entry.id);
          c.status = statusAnterior;
          saveData();
          refreshDashboard();
          if (detailCallId === callId) openDetail(callId);
        }
        showToast('Atendimento removido.');
        if (sb && c) cloudUpsert(c).then(ok => setSyncStatus(ok ? 'synced' : 'offline'));
      }
    });

    if (sb) {
      cloudUpsert(call).then(ok => {
        setSyncStatus(ok ? 'synced' : 'offline');
        if (!ok) showToast('Salvo neste aparelho. Sem conexão para sincronizar agora.', true);
      });
    }
  ...
  function deleteInteracao(callId, interacaoId) {
    const call = calls.find(c => c.id === callId);
    if (!call) return;
    openConfirm({
      title: 'Apagar este atendimento?',
      message: 'Este atendimento adicional será removido desta chamada. Esta ação não pode ser desfeita.',
      okLabel: 'Apagar',
      danger: true,
      onConfirm: () => {
        call.interacoes = (call.interacoes || []).filter(it => it.id !== interacaoId);
        saveData();
        refreshDashboard();
        if (detailCallId === callId) openDetail(callId);
        showToast('Atendimento apagado.');
        if (sb) cloudUpsert(call).then(ok => setSyncStatus(ok ? 'synced' : 'offline'));
      }
    });
  }
  ...
  /* ---------------------------------------------------------------
     NAVEGAÇÃO ENTRE VIEWS (Dashboard / Financeiro)
  --------------------------------------------------------------- */
  function switchView(viewId) {
    currentView = viewId;
    document.querySelectorAll('.view').forEach(v => v.classList.toggle('active', v.id === viewId));
    document.querySelectorAll('.main-nav-btn').forEach(b => b.classList.toggle('active', b.dataset.view === viewId));
    if (viewId === 'view-financeiro') renderFinanceiro();
  }

  /* ---------------------------------------------------------------
     FINANCEIRO — calendário mensal (peças, mão de obra, despesas...)
     -------------------------------------------------------------
     Receita das chamadas vem direto do orçamento de cada chamada
     (visita + mão de obra + peças - desconto), agrupada pela data
     do atendimento. Além disso, o usuário pode lançar manualmente
     entradas/saídas extras (ex: compra de peça para estoque,
     combustível, ferramenta) que não estão ligadas a uma chamada.
  --------------------------------------------------------------- */
  ...
  function monthBounds(date) {
    ...
    return { year, monthIndex, daysInMonth, firstWeekday, prefix };
  }

  function getCallsTotalsByDate(iso) {
    const dayCalls = calls.filter(c => c.data === iso);
    return dayCalls.reduce((acc, c) => {
      const o = c.orcamento || {};
      acc.visita += parseNumber(o.valorVisita);
      acc.maoDeObra += parseNumber(o.valorMaoDeObra);
      acc.pecas += parseNumber(o.valorPecas);
      acc.desconto += parseNumber(o.desconto);
      acc.total += parseNumber(o.valorTotal);
      acc.count += 1;
      return acc;
    }, { visita: 0, maoDeObra: 0, pecas: 0, desconto: 0, total: 0, count: 0, calls: dayCalls });
  }

  function getLedgerByDate(iso) {
    return financas.filter(f => f.data === iso);
  }

  function computeMonthSummary(date) {
    const { prefix } = monthBounds(date);
    const monthCalls = calls.filter(c => (c.data || '').startsWith(prefix));
    const monthLedger = financas.filter(f => (f.data || '').startsWith(prefix));

    const totals = monthCalls.reduce((acc, c) => {
      const o = c.orcamento || {};
      acc.visita += parseNumber(o.valorVisita);
      acc.maoDeObra += parseNumber(o.valorMaoDeObra);
      acc.pecas += parseNumber(o.valorPecas);
      acc.desconto += parseNumber(o.desconto);
      acc.receita += parseNumber(o.valorTotal);
      return acc;
    }, { visita: 0, maoDeObra: 0, pecas: 0, desconto: 0, receita: 0 });

    const entradasManual = monthLedger.filter(f => f.tipo === 'entrada').reduce((s, f) => s + parseNumber(f.valor), 0);
    const saidasManual = monthLedger.filter(f => f.tipo === 'saida').reduce((s, f) => s + parseNumber(f.valor), 0);
    const saldo = totals.receita + entradasManual - saidasManual;
    const atendimentos = monthCalls.length;
    const ticketMedio = atendimentos ? totals.receita / atendimentos : 0;

    return { ...totals, entradasManual, saidasManual, saldo, atendimentos, ticketMedio };
  }

  function updateFinMonthLabel() {
    const { year, monthIndex } = monthBounds(finCursorDate);
    document.getElementById('finMonthLabel').textContent = `${MONTHS_PT[monthIndex]} de ${year}`;
  }

  function renderFinStats() {
    const s = computeMonthSummary(finCursorDate);
    const prevMonth = new Date(finCursorDate.getFullYear(), finCursorDate.getMonth() - 1, 1);
    const prevSummary = computeMonthSummary(prevMonth);
    let variacao = null;
    if (prevSummary.receita > 0) {
      variacao = ((s.receita - prevSummary.receita) / prevSummary.receita) * 100;
    }
    const variacaoTxt = variacao === null ? '—' : `${variacao >= 0 ? '+' : ''}${variacao.toFixed(1)}%`;
    const variacaoColor = variacao === null ? 'var(--text-dim)' : (variacao >= 0 ? 'var(--success)' : 'var(--danger)');

    const cards = [
      { label: 'Peças (chamadas)', value: formatCurrency(s.pecas), icon: 'fa-gears', color: 'var(--accent)' },
      { label: 'Mão de obra', value: formatCurrency(s.maoDeObra), icon: 'fa-screwdriver-wrench', color: 'var(--accent-2)' },
      { label: 'Visitas', value: formatCurrency(s.visita), icon: 'fa-car', color: 'var(--accent)' },
      { label: 'Descontos concedidos', value: formatCurrency(s.desconto), icon: 'fa-tag', color: 'var(--danger)' },
      { label: 'Despesas/saídas extras', value: formatCurrency(s.saidasManual), icon: 'fa-arrow-trend-down', color: 'var(--danger)' },
      { label: 'Entradas extras', value: formatCurrency(s.entradasManual), icon: 'fa-arrow-trend-up', color: 'var(--accent-2)' },
      { label: 'Saldo líquido do mês', value: formatCurrency(s.saldo), icon: 'fa-scale-balanced', color: s.saldo >= 0 ? 'var(--success)' : 'var(--danger)' },
      { label: 'Ticket médio', value: formatCurrency(s.ticketMedio), icon: 'fa-receipt', color: 'var(--accent)' },
      { label: 'Atendimentos no mês', value: String(s.atendimentos), icon: 'fa-clipboard-list', color: 'var(--accent)' },
      { label: 'Vs. mês anterior', value: variacaoTxt, icon: 'fa-chart-line', color: variacaoColor }
    ];

    document.getElementById('finStatsGrid').innerHTML = cards.map(c => `
      <div class="stat-card" style="--stat-color:${c.color}">
        <i class="fa-solid ${c.icon}"></i>
        <span class="stat-value">${c.value}</span>
        <span class="stat-label">${c.label}</span>
  ...
  function renderFinCalendar() {
    const { year, monthIndex, daysInMonth, firstWeekday } = monthBounds(finCursorDate);
    const todayIso = todayISO();
    const grid = document.getElementById('finCalendarGrid');
    let html = '';

    for (let i = 0; i < firstWeekday; i++) {
      html += `<div class="fin-day-cell fin-day-empty"></div>`;
    }

    for (let day = 1; day <= daysInMonth; day++) {
      const iso = isoFromParts(year, monthIndex, day);
      const callTotals = getCallsTotalsByDate(iso);
      const ledger = getLedgerByDate(iso);
      const entradaManual = ledger.filter(f => f.tipo === 'entrada').reduce((s, f) => s + parseNumber(f.valor), 0);
      const saidaManual = ledger.filter(f => f.tipo === 'saida').reduce((s, f) => s + parseNumber(f.valor), 0);

      let badges = '';
      if (callTotals.total > 0) badges += `<span class="fin-day-badge receita">${formatCurrency(callTotals.total)}</span>`;
      if (entradaManual > 0) badges += `<span class="fin-day-badge entrada">+${formatCurrency(entradaManual)}</span>`;
      if (saidaManual > 0) badges += `<span class="fin-day-badge saida">-${formatCurrency(saidaManual)}</span>`;

      const isToday = iso === todayIso;
      const isSelected = iso === finSelectedDay;
      html += `
        <button type="button" class="fin-day-cell${isToday ? ' fin-day-today' : ''}${isSelected ? ' fin-day-selected' : ''}" data-iso="${iso}">
          <span class="fin-day-num">${day}</span>
          <div class="fin-day-badges">${badges}</div>
        </button>
      `;
    }

    grid.innerHTML = html;
    grid.querySelectorAll('.fin-day-cell:not(.fin-day-empty)').forEach(cell => {
      cell.addEventListener('click', () => openFinDay(cell.dataset.iso));
    });
  }

  function openFinDay(iso) {
    finSelectedDay = iso;
    renderFinCalendar();

    const callTotals = getCallsTotalsByDate(iso);
    const ledger = getLedgerByDate(iso);
    const panel = document.getElementById('finDayPanel');
    panel.style.display = 'block';
    document.getElementById('finDayTitle').textContent = `Dia ${formatDateBR(iso)}`;

    let html = '';

    html += `<div class="fin-day-section-title">Chamadas do dia</div>`;
    if (callTotals.calls.length === 0) {
      html += `<div class="fin-empty-hint">Nenhuma chamada com data neste dia.</div>`;
    } else {
      html += `<div class="fin-day-calls">` + callTotals.calls.map(c => `
        <div class="fin-row" data-call-id="${c.id}" style="cursor:pointer;">
          <div class="fin-row-main">
            <b>${escapeHtml(c.numero)} — ${escapeHtml((c.cliente && c.cliente.nome) || 'Sem nome')}</b>
            <span>${escapeHtml(c.status)} · Peças ${formatCurrency(c.orcamento.valorPecas)} · Mão de obra ${formatCurrency(c.orcamento.valorMaoDeObra)}</span>
          </div>
          <div class="fin-row-value"><b class="positivo">${formatCurrency(c.orcamento.valorTotal)}</b></div>
        </div>
      `).join('') + `</div>`;
    }

    html += `<div class="fin-day-section-title">Lançamentos manuais</div>`;
    if (ledger.length === 0) {
      html += `<div class="fin-empty-hint">Nenhum lançamento manual neste dia.</div>`;
    } else {
      html += `<div class="fin-day-ledger">` + ledger.map(f => `
        <div class="fin-row">
          <div class="fin-row-main">
            <b>${escapeHtml(f.categoria)}</b>
            <span>${escapeHtml(f.descricao || '—')}</span>
          </div>
          <div class="fin-row-value">
            <b class="${f.tipo === 'entrada' ? 'positivo' : 'negativo'}">${f.tipo === 'entrada' ? '+' : '-'}${formatCurrency(f.valor)}</b>
            <button type="button" class="icon-btn fin-ledger-del" data-id="${f.id}" title="Excluir"><i class="fa-solid fa-trash"></i></button>
          </div>
        </div>
      `).join('') + `</div>`;
    }

    document.getElementById('finDayContent').innerHTML = html;

    document.getElementById('finDayContent').querySelectorAll('.fin-row[data-call-id]').forEach(row => {
      row.addEventListener('click', () => openDetail(row.dataset.callId));
    });
    document.getElementById('finDayContent').querySelectorAll('.fin-ledger-del').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        deleteLancamento(btn.dataset.id);
      });
    });
  }

  function closeFinDay() {
    finSelectedDay = null;
    document.getElementById('finDayPanel').style.display = 'none';
    renderFinCalendar();
  }

  function renderFinLedgerList() {
    const { prefix } = monthBounds(finCursorDate);
    const monthLedger = financas
      .filter(f => (f.data || '').startsWith(prefix))
      .sort((a, b) => (a.data < b.data ? 1 : -1));

    const list = document.getElementById('finLedgerList');
    if (monthLedger.length === 0) {
      list.innerHTML = `<div class="fin-empty-hint">Nenhum lançamento manual neste mês. Use "Lançamento manual" para registrar despesas ou receitas extras (peças de estoque, combustível, ferramentas, etc.).</div>`;
      return;
    }

    list.innerHTML = monthLedger.map(f => `
      <div class="fin-ledger-item">
        <div class="fin-ledger-info">
          <b>${escapeHtml(f.categoria)} · ${formatDateBR(f.data)}</b>
          <span>${escapeHtml(f.descricao || '—')}</span>
          <span class="fin-ledger-tag ${f.tipo}">${f.tipo === 'entrada' ? 'Entrada' : 'Saída'}</span>
        </div>
        <div class="fin-ledger-actions">
          <b class="${f.tipo === 'entrada' ? 'positivo' : 'negativo'}" style="color:${f.tipo === 'entrada' ? 'var(--success)' : 'var(--danger)'}">${f.tipo === 'entrada' ? '+' : '-'}${formatCurrency(f.valor)}</b>
          <button type="button" class="fin-ledger-del" data-id="${f.id}" title="Excluir"><i class="fa-solid fa-trash"></i></button>
        </div>
      </div>
    `).join('');

    list.querySelectorAll('.fin-ledger-del').forEach(btn => {
  ...
  function renderFinanceiro() {
    ...
    renderFinCalendar();
    renderFinLedgerList();
    if (finSelectedDay) openFinDay(finSelectedDay);
  }

  function changeFinMonth(delta) {
    finCursorDate = new Date(finCursorDate.getFullYear(), finCursorDate.getMonth() + delta, 1);
    finSelectedDay = null;
    document.getElementById('finDayPanel').style.display = 'none';
    renderFinanceiro();
  }

  function goToFinToday() {
    finCursorDate = new Date();
    finSelectedDay = null;
    document.getElementById('finDayPanel').style.display = 'none';
    renderFinanceiro();
  }

  function openLancamentoModal(prefillDate) {
    finEditingLancamentoId = null;
    document.getElementById('ln_id').value = '';
    document.getElementById('ln_data').value = prefillDate || finSelectedDay || todayISO();
    document.getElementById('ln_tipo').value = 'saida';
    document.getElementById('ln_categoria').value = FIN_CATEGORIAS[0];
    document.getElementById('ln_valor').value = '';
    document.getElementById('ln_descricao').value = '';
    document.getElementById('lancamentoModalOverlay').classList.add('open');
  }

  function closeLancamentoModal() {
    document.getElementById('lancamentoModalOverlay').classList.remove('open');
  }

  function saveLancamento() {
    const data = document.getElementById('ln_data').value;
    const valor = parseNumber(document.getElementById('ln_valor').value);

    if (!data) { showToast('Informe a data do lançamento.', true); return; }
    if (!valor || valor <= 0) { showToast('Informe um valor maior que zero.', true); return; }

    const lancamento = {
      id: finEditingLancamentoId || uid(),
      data,
      tipo: document.getElementById('ln_tipo').value,
      categoria: document.getElementById('ln_categoria').value,
      descricao: document.getElementById('ln_descricao').value.trim(),
      valor
    };

    const idx = financas.findIndex(f => f.id === lancamento.id);
    if (idx >= 0) financas[idx] = lancamento;
    else financas.push(lancamento);

    if (!saveFinancas()) return;
  ...
  function exportFinCsv() {
    const { prefix, year, monthIndex } = monthBounds(finCursorDate);
    const monthCalls = calls.filter(c => (c.data || '').startsWith(prefix));
    const monthLedger = financas.filter(f => (f.data || '').startsWith(prefix));

    const rows = [['Data', 'Tipo', 'Origem', 'Descrição', 'Peças', 'Mão de obra', 'Visita', 'Desconto', 'Valor']];

    monthCalls.forEach(c => {
      const o = c.orcamento || {};
      rows.push([
        c.data, 'Receita', `Chamada ${c.numero}`,
        (c.cliente && c.cliente.nome) || '',
        o.valorPecas || 0, o.valorMaoDeObra || 0, o.valorVisita || 0, o.desconto || 0, o.valorTotal || 0
      ]);
    });

    monthLedger.forEach(f => {
      rows.push([
        f.data, f.tipo === 'entrada' ? 'Entrada manual' : 'Saída manual', f.categoria,
        f.descricao || '', '', '', '', '', f.valor
      ]);
    });
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `financeiro_${year}-${pad2(monthIndex + 1)}.csv`;
    document.body.appendChild(a);
    a.click();
  ...
  function bindEvents() {
    ...
    // navegação principal (Dashboard / Financeiro)
    document.querySelectorAll('.main-nav-btn').forEach(btn => {
      btn.addEventListener('click', () => switchView(btn.dataset.view));
    });

    // financeiro — calendário
    document.getElementById('finPrevMonth').addEventListener('click', () => changeFinMonth(-1));
    document.getElementById('finNextMonth').addEventListener('click', () => changeFinMonth(1));
    document.getElementById('finTodayBtn').addEventListener('click', goToFinToday);
    document.getElementById('finDayClose').addEventListener('click', closeFinDay);
    document.getElementById('finDayAddBtn').addEventListener('click', () => openLancamentoModal(finSelectedDay));
    document.getElementById('btnAddLancamento').addEventListener('click', () => openLancamentoModal(null));
    document.getElementById('btnExportFinCsv').addEventListener('click', exportFinCsv);

    // financeiro — modal de lançamento manual
    document.getElementById('btnCloseLancamento').addEventListener('click', closeLancamentoModal);
    document.getElementById('btnCancelLancamento').addEventListener('click', closeLancamentoModal);
    document.getElementById('btnSaveLancamento').addEventListener('click', saveLancamento);
    document.getElementById('lancamentoModalOverlay').addEventListener('click', e => {
      if (e.target.id === 'lancamentoModalOverlay') closeLancamentoModal();
    });
    ...
    document.getElementById('btnNewSamePersonFromDetail').addEventListener('click', scrollToInteracoes);
    ...
    document.addEventListener('keydown', e => {
      ...
      else if (document.getElementById('lancamentoModalOverlay').classList.contains('open')) closeLancamentoModal();
    });
  }
  ...
  function init() {
    ...
    loadFinancas();           // lançamentos manuais do financeiro (local)
    populateMarcaFilter();
    bindEvents();
    refreshDashboard();
